...
        digitosDescifrados.push(digitoOriginal);
    }
    
    let textoDescifrado = '';
    let i = 0;
    
    while (i < digitosDescifrados.length) {
        let codigoASCII = undefined;
        let strCodigo = '';

        if (i + 2 < digitosDescifrados.length) {
            strCodigo = digitosDescifrados[i].toString() + digitosDescifrados[i+1].toString() + digitosDescifrados[i+2].toString();
            const posibleCodigo = parseInt(strCodigo);

            if (posibleCodigo >= 80 && posibleCodigo <= 126) { 
                codigoASCII = posibleCodigo;
                i += 3;
            }
        }

        if (codigoASCII === undefined && i + 1 < digitosDescifrados.length) {
            strCodigo = digitosDescifrados[i].toString() + digitosDescifrados[i+1].toString();
            codigoASCII = parseInt(strCodigo);
            i += 2;
        }
        
        if (codigoASCII === undefined) {
             return `Error: Secuencia de dígitos inválida o incompleta en la posición ${i}.`;
        }
...
    if (vocabulario.length > 0) {
        const cifradoBtn = document.getElementById('cifradoBtn');
        const descifradoBtn = document.getElementById('descifradoBtn');

        if (cifradoBtn) cifradoBtn.addEventListener('click', ejecutarCifrado);
        if (descifradoBtn) descifradoBtn.addEventListener('click', ejecutarDescifrado);
        
        if (document.getElementById('claveInput')) {
            ejecutarCifrado();
        }
    } else {
        console.error("Error crítico: El vocabulario no contiene palabras.");
    }
}
...
>>>>>>> 767d656dc86347584736c24fe7d9f1fe831ca732
inicializar();